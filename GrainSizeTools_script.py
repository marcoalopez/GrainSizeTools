...
def extract_areas(file_path='auto', col_name='Area'):
    ...
    tabular-like text data generated by the ImageJ or similar applications.

    PARAMETERS

    file_path:
    the file location in the OS in quotes (e.g. 'C:/yourFileLocation/nameOfTheFile.csv')
    If 'auto' (the default) the function will ask you for the location of the file
...
def calc_diameters(areas, correct_diameter=0):
    ...
    areas:
    The sectional areas of the grains. A numpy array.

    correct_diameter:
    This adds the width of the grain boundaries to correct the diameters. If
    correct_diameter is not declared, no correction is considered. A float or
    integer.
    """
...
def find_grain_size(areas, diameters, plot='lin', binsize='auto'):
    ...
    of apparent diameters and their areas. It includes the mean, area-weighted
    mean, median, and frequency peak grain sizes.

    PARAMETERS

    areas:
    The areas of the grain profiles. A numpy array.

    diameters:
    The apparent diameters of the grains. A numpy array.

    plot:
    the scale/type of the plot and grain size estimation. This can
    be 'lin' for a linear frequency vs diameter, 'log' for a
    frequency vs logarithmic diameter, 'sqrt' for a frequency
    vs square root diameter, and 'area' for a area-weighted
    frequency vs diameter.

    binsize:
    if a string, it defines the plug-in method to calculate the bin size. The
    methods can be 'auto', 'fd' (Freedman-Diaconis rule), 'doane' (Doane's rule),
    'scott' (Scott rule), 'sturges' (Sturge's rule). If not specified, the 'auto'
    rule is used by default. When integer or float, it directly specifies the
    bin size. 
    """
...
def derive3D(diameters, numbins=10, set_limit=None, fit=False, initial_guess=False):
    ...
    diameters:
    The apparent diameters of the grains. A numpy array or Python list. 
    ...
    # check if numbins is an integer
...
def Saltykov_plot(left_edges, freq3D, binsize, mid_points, cdf_norm):
    """ Generate two plots once the Saltykov method is applied: i) A frequency
        plot, and ii) a volume-weighted cumulative frequency plot"""
...
def calc_freq_grainsize(diameters, binsize, plot='freq'):
    """ Calculate the distribution of grain sizes using the histogram and the
    Gaussian kernel density estimator. It returns the modal interval, middle value
    of modal interval, and Gaussian kernel density estimation peak. Then call the
    function responsible for generating the corresponding plot.

    PARAMETERS

    diameters:
    The diameters of the grains. A numpy array.

    binsize:
    The bin size. A string (rule of thumb), integer or float.
...
def calc_areaweighted_grainsize(areas, diameters, binsize):
    ...
    binsize:
    The bin size. A string (when the definition is a method), integer, or float
    """
