...
def get_column(file_path='auto', col_name='Area'):
    ...
    col_name : string, optional
        the name of the column to extract. It is 'Area' by default.
...
def calc_grain_size(diameters, areas=None, plot='lin', binsize='auto'):
...
def Saltykov(diameters, numbins=10, calc_vol=None, text_file=None, return_data=False):
...
def fit_log(x, y, initial_guess):
    """ Fits a lognormal distribution to...
    It uses the curve_fit scipy procedure that is an implementation
    of the Mard... algorithm...
    ...
    initial_guess : tuple or list with two values
        a tuple or list with two starting values that

    Assumptions
    -----------
    the points aproximatelly follows a lognormal distribution
...
def calc_freq_grainsize(diameters, binsize, plot):
    ...
    if plot == 'log':
        xgrid = gen_xgrid(diameters, 1, diameters.max() + binsize)
    else:
    ...
    print(' ')
    print('Peak grain size (based on KDE) = {} microns' .format(round(x_peak, 2)))
    print('KDE bandwidth = {} (Silverman rule)' .format(round(kde.covariance_factor() * diameters.std(ddof=1), 2)))
    print(' ')
...
==================  ==================================================================
get_column          Extract data from tabular-like text files (txt, csv or xlsx)
...
calc_shape          Characterize the shape of the actual grain size distribution
