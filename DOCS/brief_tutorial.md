...
to set a user-defined bin size (in this example set to ten). The user-defined bin size can be of type integer or float (*i.e.* an irrational number). Freedman-Diaconis rule is a better option compared to Scott rule when it is suspected that there are outliers in the population of grain profile sizes.
