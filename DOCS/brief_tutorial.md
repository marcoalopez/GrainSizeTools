...
> **Note**
> This tutorial assumes that you know nothing about Python programming language. This means that you do not need any previous knowledge of the Python programming language to use the script and get the results. Just follow the instructions.

### *Running the script*

Once you have the latest version of the GrainSizeTools script and the required software installed in your system (see [requirements](https://github.com/marcoalopez/GrainSizeTools/blob/master/DOCS/Requirements.md)), open the script in a Python editor. For example the Canopy editor -if you installed the Enthought Canopy package-, the IDLE or the Spyder editor -if you installed the Anaconda package-. Then, you need to run the code. For this, in the Canopy or the Spyder editor just click on the green play icon or go to the *Run* menu and click on “Run file” (Fig. 1). If you open the script in the IDLE, just press F5 or go to the *Run menu* and click on “Run module” (Fig. 1).
...
Note that contrary to what was shown so far, the function is called directly in the Python shell since it is no longer necessary to store any data into a variable. The inputs are the arrays with the areas and diameters previously stored and, if desired, the bin size. The function includes two plug-in methods to estimate an 'optimal' bin size, the Scott (Scott, 1979) and the Freedman-Diaconis (Freedman and Diaconis, 1981) rules. The ```find_grain_size``` function uses by default the Scott rule. If you want to use the Freedman-Diaconis rule or an user-defined bin size value, it can be do it as follows:
...
to set a user-defined bin size (in this example set to ten). The user-defined bin size can be of type integer or float (*i.e.* an irrational number). Freedman-Diaconis rule is a better option compared to Scott rule when we suspect that there are outliers in the population of apparent grain sizes.

After pressing the Enter key, the function will return a number of different 1D measures of grain size typically used in paleopiezometry studies, including the mean, the median, the area-weighted mean and the frequency peak grain sizes (see details in [Lopez-Sanchez and Llana-Fúnez 2015](http://www.solid-earth.net/6/475/2015/se-6-475-2015.html)). Other parameters of interest are also provided, such as the bin size estimated (also indicating the method used in the estimation) and the bandwidth used for the Gaussian kernel density estimator according to the Silverman rule (Silverman 1986). As stated in [Lopez-Sanchez and Llana-Fúnez 2015](http://www.solid-earth.net/6/475/2015/se-6-475-2015.html), **a minimum of 433 measured grain profiles are needed to yield consistent results**, although we recommend to measure at least 965 if possible.
...
```
Since the Saltykov method is based on the discretization of the grain size distribution, the inputs are an array with the apparent diameters of the grains and the desired number of bins/classes of the histogram. If the number of bins is not declared is set to ten by default. To define another number of classes, the user can use any positive **integer** value. However, it is advisable to choose a number between 10 and 20 (see later for details). After pressing the Enter key, the function will return the bin size estimated and an array with the normalized frequencies of the different classes. Also, a new window with two plots will appear (Fig 7). At left, a frequency plot showing the estimated 3D grain size distribution in a form of a histogram and, at right, a volume-weighted cumulative density curve. The latter allow the user to estimate graphically which percentage of volume is occupied by a defined fraction of grain sizes. In case we want to estimate quantitatively the volume of a particular grain fraction (i.e. the volume occupied by a fraction of grains less or equal to a certain value) we need to add a new parameter within the function as follows (*Note: this feature is available since version 1.0*):
...
Note that in the case we include a new parameter named ```fit``` that it is set to ```True``` (it is ```False``` by default). Sometimes, the least squares algorithm will fail at fitting the lognormal distribution to the unfolded data (e.g. Fig. 8). This is due to the procedure used to find the optimal MSD and scale values, based on the Levenberg–Marquardt algorithm (Marquardt, 1963), only converges to the global minimum when their initial guesses are already somewhat close to the final solution. Based on experience, this initial guesses were set by default at 1.2 and 25.0 for the MSD and scale values, respectively. However, when the algorithm fails it is necessary to change these default values by adding a new parameter in the ```derive3D``` function as follows:

```python
>>> derive3D(diameters, numbins=15, set_limit=None, fit=True, initial_guess=True)
```
When the ```initial_guess``` parameter is set to ```True```, the script will ask you about the new guess values for both parameters (it also indicates which the default ones are). Based on our experience, a wise strategy is to let the shape value in its default value (1.2) and decrease the scale value every five units until the fitting procedure yield a coherent result (*e.g.* 25 -> 20 -> 15...) (Fig. 8).
