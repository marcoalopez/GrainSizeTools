...
***I get the results but not the figures when using the Spyder IDE***  
This issue is produced because the size of the figures returned by the script are too large to show them inside the console using the **inline** mode. To fix this go to the menu bar  ```Tools>Preferences>IPython console>Graphics``` and in the *Graphics backend* select *Automatic*.
